        ...
        for b in range(B):
            ...
            for c in range(self.num_classes):
                ...
                if gt_b_c.sum() > 0:
                    ...
                    per_class_losses.append(dice_c)
            if per_class_losses:
                # average Dice over present classes for this image
                image_losses.append(torch.stack(per_class_losses).mean())
        if image_losses:
            # average across images
            return self.lambda_term * torch.stack(image_losses).mean()
        # no classes present in any image
