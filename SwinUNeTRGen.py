...
class SwinUNETRMaskGen:
    def __init__(self, weights_path, full_size=True, device='cuda:1'):
        ...
        """
        print(device)
        # Instantiate the model architecture using the custom definition
        # Setting out_channels=2 to match the loaded checkpoint's structure
        self.model = SwinUNETR( # Now uses the custom SwinUNETR
            img_size=(96, 96, 96), # Keep placeholder, check if custom __init__ needs it
            in_channels=1,
            out_channels=2, # Set to 2 to match checkpoint structure
            feature_size=24, # Updated to match user's training
            deep_supervision=True, # Enabled to match saved checkpoint keys
            use_v2=True # Enabled to match checkpoint keys
            # Add other necessary args for custom SwinUNETR if its __init__ differs significantly
        )

        # Load the state dictionary directly
        checkpoint = torch.load(weights_path, map_location=device, weights_only=False)

        # Attempt to load the state dict - check common keys if direct load fails
        if isinstance(checkpoint, dict) and 'state_dict' in checkpoint:
            state_dict = checkpoint['state_dict']
        elif isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
            state_dict = checkpoint['model_state_dict']
        else:
            # Assume the entire checkpoint file is the state_dict
...
class SwinUNETRMaskGen:
    def __init__(self, weights_path, full_size=True, device='cuda:1'):
        ...
        print(device)
        # Instantiate the model architecture using the custom definition
        # Reverting out_channels to 2 based on persistent checkpoint shape errors
        self.model = SwinUNETR( # Now uses the custom SwinUNETR
            img_size=(96, 96, 96), # Keep placeholder, check if custom __init__ needs it
            in_channels=1,
            out_channels=2, # Reverted to 2 based on checkpoint error shapes
            feature_size=24, # Updated to match user's training
            deep_supervision=True, # Enabled to match saved checkpoint keys
            use_v2=True # Enabled to match checkpoint keys
            # Add other necessary args for custom SwinUNETR if its __init__ differs significantly
